...
def main():
    ...
    if not args.skip_scrape:
        with timed("Step 1: Query-driven data collection"):
            from qubettera.rag.collection import run as collect_run
            docs = collect_run()
            print(f"Collected {len(docs)} pages -> data/raw_docs.jsonl")

    with timed("Step 2: Cleaning & filtering"):
        from qubettera.rag.clean import run as clean_run
        clean_run()

    with timed("Step 3: Chunking"):
        from qubettera.rag.chunk import run as chunk_run
        chunk_run()

    with timed("Step 4: Embedding generation"):
        from qubettera.rag.embed import run as embed_run
        embed_run()

    with timed("Step 5: Store in PostgreSQL"):
        from qubettera.rag.store import run as store_run
        store_run()
